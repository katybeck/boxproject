...
function mtScore(entry_id) {
    ...
    var url = 'http://boxproject.katybeck.com/cgi-bin/mt/mt-cp.cgi';
    ...
    xh.send( '__mode=vote&blog_id=7&f=scored,count&jsonp=mtScore_cb&id=' + entry_id);
    return false;
}
...
function mtUpdateScores() {
    ...
    xh.setRequestHeader( 'Content-Type', 'application/x-www-form-urlencoded' );
    xh.send( '__mode=score&blog_id=7&f=scored,count&jsonp=mtScore_cb&id=' + entry_ids);
    return false;
}
...
function favorite(evt, entry_id, score_span) {
    ...
    xh.setRequestHeader( 'Content-Type', 'application/x-www-form-urlencoded' );
    xh.send( '__mode=vote&blog_id=7&f=scored,sum&jsonp=favorite_cb&id=' + entry_id);
    var span = document.getElementById(score_span);
    if (span) span.innerHTML = '<img src="http://boxproject.katybeck.com/mt4.2-static/images/indicator.white.gif" height="10" width="10" alt="favoriting..." />';
...
function script_follow(id) {
    ...
    var xh = mtGetXmlHttp();
    if (!xh) return false;
    xh.open('POST', 'http://boxproject.katybeck.com/cgi-bin/mt/mt-cp.cgi', true);
    xh.onreadystatechange = function() {
        if ( xh.readyState == 4 ) {
            if (xh.status && ( xh.status != 200 ) ) {
                // error - ignore
    ...
    if (span) span.innerHTML = '<img src="http://boxproject.katybeck.com/mt4.2-static/images/indicator.white.gif" height="10" width="10" alt="Following..." />';
}
...
function script_leave(id) {
    ...
    var span = document.getElementById('following-status');
    if (span) span.innerHTML = '<img src="http://boxproject.katybeck.com/mt4.2-static/images/indicator.white.gif" height="10" width="10" alt="Leaving..." />';
}
...
function mtFetchUser(cb) {
    ...
    if ( ( cb == 'mtSetUser' ) && mtGetUser() ) {
        ...
        script.src = 'http://boxproject.katybeck.com/cgi-bin/mt/mt-comments.cgi?__mode=session_js&blog_id=7&jsonp=' + cb + '&ts=' + ts;
...
function mtCommentOnSubmit(f) {
    if (!mtRequestSubmitted) {
        ...
        if (f.armor)
            f.armor.value = '12f14e78034470df07805bfca4879a84acc78728';
...
function mtSignIn() {
    ...
    var url = 'http://boxproject.katybeck.com/cgi-bin/mt/mt-cp.cgi?__mode=login&blog_id=7';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
        } else {
            url += '&return_url=http%3A%2F%2Fboxproject.katybeck.com%2F';
        }
    } else {
...
function mtSignOut(entry_id) {
    ...
    var doc_url = document.URL;
    doc_url = doc_url.replace(/#.+/, '');
    var url = 'http://boxproject.katybeck.com/cgi-bin/mt/mt-cp.cgi?__mode=logout&blog_id=7';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
...
function mtShowGreeting() {
    ...
    if ( u && u.is_authenticated ) {
        if ( u.is_banned ) {
            ...
            var user_link;
            if ( u.is_author ) {
                user_link = '<a href="http://boxproject.katybeck.com/cgi-bin/mt/mt-cp.cgi?__mode=edit&amp;return_to=' + encodeURIComponent(document.URL) + '&amp;blog_id=7';
                user_link += '">' + u.name + '</a>';
            } else {
...
function fadeInByTag(tag) {
	
		$(".ataglance div").fadeTo("fast", 0.45);
		
		$(".ataglance ." + tag).fadeTo("fast", 1.00);
	}
